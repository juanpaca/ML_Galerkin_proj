#!/usr/bin/env python3
"""Train on 1000-sample dataset — quick iteration."""
...
xi_t = torch.tensor(xi_s, dtype=torch.float32)

for mode in ds["mode_names"]:
    print(f"\n{'='*50}\n{mode}\n{'='*50}")
    ...
    p_t = torch.tensor(tr["pe"], dtype=torch.float32)
    r_t = torch.tensor(tr["rho"], dtype=torch.float32)
    b_t = torch.tensor(b_t, dtype=torch.float32)
    p_v = torch.tensor(va["pe"], dtype=torch.float32)
    r_v = torch.tensor(va["rho"], dtype=torch.float32)
    b_v = torch.tensor(b_v, dtype=torch.float32)

    model = KANBubble1D(n_hidden=N_HIDDEN, n_grid=8, spline_order=3)
    ...
    for ep in range(N_EPOCHS):
        ...
        for s in range(0, n_t, BATCH_SIZE):
            ...
            opt.step()
            el += F.mse_loss(pr, b_t[bi]).item() * bs
        el /= n_t
    ...
    # Test on full FD grid
    model.load_state_dict(best_st); model.eval()
    xi_ft = torch.tensor(xi_f, dtype=torch.float32)
    p_te = torch.tensor(te["pe"], dtype=torch.float32)
    r_te = torch.tensor(te["rho"], dtype=torch.float32)
    ...
    with torch.no_grad():
        ...
        b_te_t = torch.tensor(te["b"], dtype=torch.float32)
        te_l = F.mse_loss(pr_t, b_te_t).item()
        rms = np.sqrt(np.mean((pr_t.numpy() - te["b"])**2, axis=1))
