...
- **Evaluation on shared CUDA nodes**: node NVML often broken (`nvidia-smi`: Driver/library mismatch → torch warns "Can't initialize NVML"). A cudaMalloc OOM then surfaces as `NVML_SUCCESS ... INTERNAL ASSERT ... CUDACachingAllocator.cpp:963` instead of a clean OOM error (the OOM reporter wants NVML data). Workaround in `tutorial_darcy_variable.py`: keep eval `chunk_size=16` (large first chunks reserve big CUDA segments that OOM on crowded nodes; once a small chunk establishes the pool, steady-state chunks reuse it), retry once after `torch.cuda.empty_cache()`, and only then fall back to CPU (~0.4 s/profile at n_fd=6401 — slow, so re-run on a freed GPU is preferred).
